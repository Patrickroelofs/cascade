...
## 2026-07-22
- [fix] A version history entry with no content (e.g. a node's state before its first edit) now says "Empty" instead of rendering an indistinguishable blank line. [#408](https://github.com/Patrickroelofs/cascade/issues/408)
...
- [feat] Soft-deleted nodes are no longer kept forever: a new `pnpm db:purge-deleted:app` script permanently removes every currently deleted node, meant to be run periodically by self-hosted deployments (e.g. via cron) to control how long deleted nodes stay restorable. [#408](https://github.com/Patrickroelofs/cascade/issues/408)
- [feat] Deleting a node no longer erases its version history: the node (and everything under it) is now soft-deleted, so it still shows up in tree-wide and per-node version history — flagged with a "Deleted" badge — and restoring any of its versions brings back the whole subtree exactly as it was, reattaching it under its original parent or at the root if that parent is gone too. Deletion is now serialized the same way moves and creates are, so it can't race with them or with itself, keeping a restored subtree's descendants consistent even under concurrent edits or a repeated delete. [#408](https://github.com/Patrickroelofs/cascade/issues/408)
