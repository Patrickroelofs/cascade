...
## 2026-07-22
- [feat] Deleting a whole tree (a node with descendants) now shows up as a single entry in version history — previously every node in the deleted subtree got its own flagged "Deleted" row, cluttering the tree-wide timeline with one line per node instead of one per deletion. Selecting that entry shows a read-only recreation of the deleted subtree exactly as it looked in the outliner (structure, indentation, and rich content), instead of a content diff. [#408](https://github.com/Patrickroelofs/cascade/issues/408)
